...
// Helper para formatear fecha a formato input date (YYYY-MM-DD)
const toInputDate = (fecha) => {
  if (!fecha) return "";
  return new Date(fecha).toISOString().split("T")[0];
};

// Helper para calcular si un producto de tiempo limitado está activo ahora
const estaActivo = (p) => {
  if (p.tipo === "stock") return p.stock > 0;
...
export default function AdminProductos() {
  ...
  const abrirCrear = () => {
    setForm({
      ...
      imagen: "",
      tipo: "stock",
      activo: true, // stock arranca activo por defecto
      fechaInicio: "",
      fechaFin: "",
    });
    setModal("crear");
  ...
  const abrirEditar = (p) => {
    setForm({
      ...p,
      fechaInicio: toInputDate(p.fechaInicio),
      fechaFin: toInputDate(p.fechaFin),
    });
    setModal(p);
  };
  ...
  const guardar = async () => {
    ...
    try {
      if (modal === "crear") {
        ...
        await fetch(`${API}/api/productos/${modal._id}`, {
          method: "PUT",
          headers,
          body: JSON.stringify(form),
        });
  ...
  // Badge de estado para las cards del panel
  const getBadge = (p) => {
    // Si está manualmente inactivo, mostrar eso primero
    if (!p.activo) return { label: "Inactivo", color: "#888", bg: "rgba(136,136,136,0.12)" };

    if (p.tipo === "tiempo_limitado") {
      if (!p.fechaInicio || !p.fechaFin)
        return { label: "Sin fechas", color: "#888", bg: "rgba(136,136,136,0.12)" };
      const ahora = new Date();
      if (ahora < new Date(p.fechaInicio))
        return { label: "Próximamente", color: "#f0a93f", bg: "rgba(240,169,63,0.12)" };
      if (ahora > new Date(p.fechaFin))
        return { label: "Expirado", color: "#f0493f", bg: "rgba(240,73,63,0.12)" };
      return { label: "Activo", color: "#3fd68a", bg: "rgba(63,214,138,0.12)" };
    }
    if (p.stock === 0)
      return { label: "Sin stock", color: "#f0493f", bg: "rgba(240,73,63,0.12)" };
      ...
      <div style={{ padding: "36px 40px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32 }}>
          ...
          <button onClick={abrirCrear} style={{
            ...
            padding: "10px 20px", borderRadius: 6, fontFamily: "Montserrat",
          ...
          {productos.map((p, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                overflow: "hidden",
                opacity: p.activo ? 1 : 0.5, // inactivos se ven apagados
              }}>
                {/* Imagen */}
                <div style={{
                  ...
                  <span style={{
                    ...
                    background: p.tipo === "tiempo_limitado" ? "rgba(63,169,240,0.15)" : "rgba(238,238,238,0.08)",
                    color: p.tipo === "tiempo_limitado" ? "#3fa9f0" : "rgba(238,238,238,0.4)",
                ...
                <div style={{ padding: "16px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
                    <p style={{ fontFamily: "Montserrat", fontWeight: 700, fontSize: "0.9rem", color: "#eeeeee" }}>
                      {p.nombre}
                    </p>
                    {/* Badge estado */}
                    <span style={{
                  ...
                  <p style={{
                    ...
                    color: "rgba(238,238,238,0.3)", letterSpacing: "0.08em",
                    textTransform: "uppercase", marginBottom: 8,
                  }}>#{p.codigoProducto}</p>
                  <p style={{ fontFamily: "Montserrat", fontWeight: 900, fontSize: "1rem", color: "#f0493f", marginBottom: 16 }}>
      ...
      {modal && (
        <>
          ...
          <div style={{
                ...
                {key === "descripcion" ? (
                  <textarea value={form[key]}
                    ...
                    rows={3} style={{
                      ...
                      color: "#eeeeee", outline: "none", resize: "none", boxSizing: "border-box",
                    }}
                ...
                {[
                  ...
                  <button key={op.value}
                    onClick={() => setForm((p) => ({
                      ...p,
                      tipo: op.value,
                      // Al cambiar a tiempo_limitado, se desactiva automáticamente
                      // porque necesita fechas antes de ser visible en la tienda
                      activo: op.value === "tiempo_limitado" ? false : true,
                    }))}
                    style={{
                    ...
                    <p style={{
                      ...
                      color: form.tipo === op.value ? "#f0493f" : "rgba(238,238,238,0.5)", marginBottom: 2,
                    }}>{op.label}</p>
                    <p style={{ fontFamily: "Poppins", fontSize: "0.7rem", color: "rgba(238,238,238,0.3)", lineHeight: 1.4 }}>
                      {op.desc}
                    </p>
                  </button>
                ))}
            ...
            {/* Campo stock — solo si tipo === 'stock' */}
            {form.tipo === "stock" && (
            ...
            {/* Fechas — solo si tipo === 'tiempo_limitado' */}
            {form.tipo === "tiempo_limitado" && (
              <div style={{
                background: "rgba(63,169,240,0.06)", borderRadius: 6,
                border: "1px solid rgba(63,169,240,0.15)", padding: 16, marginBottom: 16,
              }}>
                <p style={{ fontFamily: "Montserrat", fontWeight: 700, fontSize: "0.72rem", color: "#3fa9f0", marginBottom: 14, letterSpacing: "0.05em" }}>
                  ⏰ Período de disponibilidad
                </p>
                <p style={{ fontFamily: "Poppins", fontSize: "0.78rem", color: "rgba(238,238,238,0.4)", marginBottom: 14 }}>
                  El producto será visible solo entre estas fechas. Sin fechas, permanece oculto.
                </p>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
                    <label style={{
                      ...
                      color: "rgba(238,238,238,0.4)", display: "block", marginBottom: 6,
                    }}>Fecha inicio</label>
                    <input type="date" value={form.fechaInicio}
                      ...
                      style={{
                        width: "100%", background: "#1e1e1e",
                        border: "1px solid rgba(238,238,238,0.1)", borderRadius: 4,
                        padding: "9px 10px", fontFamily: "Poppins", fontSize: "0.85rem",
                        color: "#eeeeee", outline: "none", boxSizing: "border-box", colorScheme: "dark",
                      }}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <button onClick={() => setForm((p) => ({ ...p, activo: !p.activo }))} style={{
                ...
                <div style={{
                  ...
                  background: form.activo ? "#3fd68a" : "rgba(238,238,238,0.15)",
                  transition: "background 0.2s", flexShrink: 0,
                }}>
                  <div style={{
                    position: "absolute", top: 3, width: 14, height: 14, borderRadius: "50%",
                    background: "#fff", transition: "left 0.2s",
                    left: form.activo ? 19 : 3,
                  }} />
                </div>
                <div>
                  <p style={{
                    fontFamily: "Montserrat", fontWeight: 700, fontSize: "0.82rem",
                    color: form.activo ? "#3fd68a" : "rgba(238,238,238,0.4)", marginBottom: 1,
                  }}>{form.activo ? "Activo" : "Inactivo"}</p>
                  <p style={{ fontFamily: "Poppins", fontSize: "0.72rem", color: "rgba(238,238,238,0.3)" }}>
                    {form.activo
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              {form.imagen ? (
                <div style={{
                  ...
                  <img src={form.imagen} alt="preview" style={{
                    width: 56, height: 56, objectFit: "contain",
                    borderRadius: 4, background: "#272727", padding: 4,
                  }} />
                  ...
                  <button onClick={() => setForm((p) => ({ ...p, imagen: "" }))} style={{
                    ...
                    color: "rgba(238,238,238,0.3)", cursor: "pointer", fontSize: "1rem", flexShrink: 0,
                  }}>✕</button>
                </div>
              ) : (
                <div style={{
      ...
      {selectorGaleria && (
        <>
          ...
          <div style={{
            ...
            <div style={{
              padding: "16px 20px", borderBottom: "1px solid rgba(238,238,238,0.06)",
              display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0,
            }}>
              <h3 style={{ fontFamily: "Montserrat", fontWeight: 800, fontSize: "1rem", color: "#eeeeee" }}>
                Seleccionar imagen{" "}
                {carpetaActual && <span style={{ color: "rgba(238,238,238,0.35)", fontWeight: 400 }}>/ {carpetaActual}</span>}
              </h3>
            ...
            <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
              {/* Sidebar carpetas */}
              <div style={{ width: 180, flexShrink: 0, borderRight: "1px solid rgba(238,238,238,0.06)", overflowY: "auto", padding: 8 }}>
                <p style={{ fontFamily: "Montserrat", fontSize: "0.62rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(238,238,238,0.25)", padding: "8px 8px 4px" }}>
                  Carpetas
                </p>
                {carpetas.length === 0
                    ...
                    {imagenes.map((img) => (
                      ...
                      >
                        ...
                        <div style={{ padding: "6px 8px" }}>
                          <p style={{ fontFamily: "Montserrat", fontSize: "0.65rem", color: "rgba(238,238,238,0.4)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
