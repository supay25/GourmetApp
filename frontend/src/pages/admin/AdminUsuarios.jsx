...
export default function AdminUsuarios() {
  ...
  const [modal, setModal] = useState(null); // null | 'crear' | objeto usuario
  ...
  // Abrir modal para editar — pre-poblar el form con los datos actuales
  const abrirEditar = (u) => {
    setForm({
      fullName: u.fullName,
      username: u.username || '',
      ...
      passwordHash: '', // no pre-poblar la contraseña por seguridad
      role: u.role || 'admin',
    });
    setFormError('');
    setModal(u);
  ...
  const guardar = async () => {
    if (!form.fullName || !form.email || !form.identification || !form.username) {
      setFormError('Nombre, usuario, email e identificación son requeridos.');
      return;
    }
    ...
    try {
      if (modal === 'crear') {
        ...
        // PUT para actualizar — omitir password si está vacío
        const body = { ...form };
        if (!body.passwordHash) delete body.passwordHash;
        const res = await fetch(`${API}/api/users/${modal._id}`, {
          method: 'PUT',
          headers,
    ...
    <AdminLayout>
      <div style={{ padding: '36px 40px' }}>
        ...
        <div style={{ background: '#272727', borderRadius: 8, border: '1px solid rgba(238,238,238,0.06)', overflow: 'hidden' }}>
          {loading ? (
            <div style={{ padding: 48, textAlign: 'center', fontFamily: 'Poppins', color: 'rgba(238,238,238,0.3)', fontSize: '0.9rem' }}>
              Cargando usuarios...
            </div>
          ) : usuarios.length === 0 ? (
            // Estado vacío
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {usuarios.map((u, i) => (
                  <tr key={u._id} style={{
                    borderBottom: i < usuarios.length - 1 ? '1px solid rgba(238,238,238,0.04)' : 'none',
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 20px', fontFamily: 'Montserrat', fontWeight: 600, fontSize: '0.85rem', color: '#f0493f' }}>
                      @{u.username || '—'}
                    </td>
                    ...
                    <td style={{ padding: '14px 20px' }}>
                      <span style={{
                        ...
                        background: roleConfig[u.role || 'admin']?.bg || roleConfig.admin.bg,
                        color: roleConfig[u.role || 'admin']?.color || roleConfig.admin.color,
                      }}>{u.role || 'admin'}</span>
                    </td>
                    <td style={{ padding: '14px 20px' }}>
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <button onClick={() => setConfirmarEliminar(u)} style={{
                          background: 'rgba(240,73,63,0.08)', border: 'none',
                          color: '#f0493f', padding: '6px 12px',
      ...
      {modal && (
        <>
          ...
          <div style={{
            ...
            {formError && (
              <p style={{ fontFamily: 'Poppins', fontSize: '0.82rem', color: '#f0493f', marginBottom: 16 }}>{formError}</p>
            )}
      ...
      {confirmarEliminar && (
        <>
          ...
          <div style={{
            ...
            <p style={{ fontFamily: 'Poppins', fontSize: '0.85rem', color: 'rgba(238,238,238,0.4)', marginBottom: 8 }}>
              {confirmarEliminar.fullName}
            </p>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => eliminar(confirmarEliminar._id)} style={{
                flex: 1, padding: '11px', borderRadius: 4,
