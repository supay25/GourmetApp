...
export default function AdminPedidos() {
  ...
  // Cancelar pedido — restaura el stock automáticamente en el backend
  ...
  return (
    <AdminLayout>
            ...
            <tbody>
              {pedidosFiltrados.map((p, i) => (
                ...
                >
                  ...
                  {/* Indicador de comprobante en la tabla */}
                  <td style={{ padding: '14px 20px' }}>
                    {p.comprobante ? (
                      <span style={{
                        fontFamily: 'Montserrat', fontWeight: 700, fontSize: '0.68rem',
                        padding: '3px 8px', borderRadius: 4,
                        background: 'rgba(63,214,138,0.12)', color: '#3fd68a',
                      }}>✓ Subido</span>
                    ) : (
                      <span style={{
                        fontFamily: 'Montserrat', fontWeight: 700, fontSize: '0.68rem',
                        padding: '3px 8px', borderRadius: 4,
                        background: 'rgba(240,169,63,0.1)', color: 'rgba(240,169,63,0.6)',
                      }}>Pendiente</span>
                    )}
                  </td>
                  <td style={{ padding: '14px 20px' }}>
      ...
      {detalle && (
        <>
          ...
          <div style={{
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {detalle.comprobante ? (
                <div style={{
                  ...
                  border: '1px solid rgba(63,214,138,0.2)',
                }}>
                  <img src={detalle.comprobante} alt="Comprobante de pago"
                  ...
                  <div style={{
                    ...
                    <span style={{ fontFamily: 'Montserrat', fontWeight: 700, fontSize: '0.72rem', color: '#3fd68a' }}>
                      ✓ Comprobante recibido
                    </span>
                    <a href={detalle.comprobante} target="_blank" rel="noopener noreferrer" style={{
                      fontFamily: 'Montserrat', fontWeight: 600, fontSize: '0.72rem',
                      color: 'rgba(238,238,238,0.4)', textDecoration: 'none', transition: 'color 0.15s',
                    }}
                ...
                // Sin comprobante — aviso al admin
                <div style={{
                  background: 'rgba(240,169,63,0.06)', border: '1px solid rgba(240,169,63,0.2)',
                  borderRadius: 6, padding: '12px 16px',
                  display: 'flex', alignItems: 'center', gap: 10,
                }}>
                  <span style={{ fontSize: '1rem' }}>⚠️</span>
                  <p style={{ fontFamily: 'Poppins', fontSize: '0.82rem', color: 'rgba(240,169,63,0.8)', margin: 0 }}>
            ...
            {/* Cambiar estado — solo si no está cancelado */}
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {['pendiente', 'confirmado', 'entregado'].map(estado => (
                    <button key={estado} onClick={() => cambiarEstado(detalle._id, estado)} style={{
                      flex: 1, padding: '10px', borderRadius: 4, textTransform: 'capitalize',
            ...
            {detalle.estado === 'cancelado' && (
              <div style={{
                ...
                borderRadius: 6, padding: '12px 16px', marginBottom: 16, textAlign: 'center',
              }}>
                <p style={{ fontFamily: 'Montserrat', fontWeight: 700, fontSize: '0.82rem', color: '#f0493f' }}>
      ...
      {confirmarCancelar && (
        <>
          <div onClick={() => setConfirmarCancelar(null)} style={{
          ...
          <div style={{
            ...
            background: '#272727', borderRadius: 8,
            border: '1px solid rgba(240,169,63,0.2)',
            ...
            <p style={{ fontFamily: 'Poppins', fontSize: '0.85rem', color: 'rgba(238,238,238,0.4)', marginBottom: 6 }}>
              {confirmarCancelar.codigo} — {confirmarCancelar.nombre}
            </p>
            <p style={{
              fontFamily: 'Poppins', fontSize: '0.8rem', color: '#f0a93f', marginBottom: 24,
              background: 'rgba(240,169,63,0.08)', border: '1px solid rgba(240,169,63,0.2)',
              borderRadius: 4, padding: '8px 12px',
            }}>
              ⚠️ El stock de los productos se restaurará automáticamente.
            </p>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setConfirmarCancelar(null)} disabled={procesando} style={{
                flex: 1, padding: '11px', borderRadius: 4,
                background: 'rgba(238,238,238,0.07)', border: 'none',
                color: 'rgba(238,238,238,0.5)', fontFamily: 'Montserrat',
                fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer',
              }}>Volver</button>
              <button onClick={() => cancelarPedido(confirmarCancelar._id)} disabled={procesando} style={{
                flex: 1, padding: '11px', borderRadius: 4,
                background: '#f0a93f', border: 'none', color: '#272727',
                fontFamily: 'Montserrat', fontWeight: 700, fontSize: '0.82rem',
                cursor: procesando ? 'not-allowed' : 'pointer', opacity: procesando ? 0.6 : 1,
              }}>
      ...
      {confirmarEliminar && (
        <>
          ...
          <div style={{
            ...
            <p style={{ fontFamily: 'Poppins', fontSize: '0.85rem', color: 'rgba(238,238,238,0.4)', marginBottom: 6 }}>
              {confirmarEliminar.codigo} — {confirmarEliminar.nombre}
            </p>
            <p style={{ fontFamily: 'Poppins', fontSize: '0.82rem', color: 'rgba(238,238,238,0.3)', marginBottom: 24 }}>
              Esta acción es permanente y no se puede deshacer.
            </p>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setConfirmarEliminar(null)} disabled={procesando} style={{
                flex: 1, padding: '11px', borderRadius: 4,
                background: 'rgba(238,238,238,0.07)', border: 'none',
                color: 'rgba(238,238,238,0.5)', fontFamily: 'Montserrat',
                fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer',
              }}>Cancelar</button>
              <button onClick={() => eliminarPedido(confirmarEliminar._id)} disabled={procesando} style={{
                flex: 1, padding: '11px', borderRadius: 4,
