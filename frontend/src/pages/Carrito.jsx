...
export default function Carrito() {
  ...
  if (items.length === 0) {
    return (
      <section style={{
        ...
        }}>Explorá nuestros productos y añadí los que te gusten.</p>
  ...
  return (
    <section style={{
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px" }}>
        ...
        <div className="carrito-layout">
          ...
          <div className="carrito-items">
            {items.map((item) => (
              <div key={item._id} style={{
                ...
                padding: "16px 20px", marginBottom: 10, transition: "border-color 0.2s",
              }}
              ...
              >
                ...
                <div style={{
                  ...
                  {item.imagen
                    ? <img src={item.imagen} alt={item.nombre} style={{ width: 46, height: 46, objectFit: "contain" }} />
                    : <i className="fa-solid fa-image" style={{ color: "rgba(238,238,238,0.15)", fontSize: "1.2rem" }} />
                  }
                </div>
                ...
                {/* Cantidad — usa item._id como identificador */}
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                  ...
                  <span style={{
                    ...
                    color: "#eeeeee", minWidth: 18, textAlign: "center",
                  }}>{item.qty}</span>
                  <button
                    onClick={() => updateQty(item._id, item.qty + 1)}
                    style={{
                ...
                {/* Precio — usa item.precio del backend */}
                <span style={{
                  fontFamily: "Montserrat", fontWeight: 800, fontSize: "0.95rem",
                  color: "#f0493f", flexShrink: 0, minWidth: 80, textAlign: "right",
                }}>₡{(item.precio * item.qty).toLocaleString()}</span>

                {/* Eliminar — usa item._id */}
                <button
                  onClick={() => removeItem(item._id)}
                  style={{
                    background: "transparent", border: "none",
                    color: "rgba(238,238,238,0.2)", cursor: "pointer",
                    fontSize: "0.9rem", padding: 4, transition: "color 0.2s", flexShrink: 0,
                  }}
          ...
          <div className="carrito-resumen" style={{
            background: "#272727", borderRadius: 6,
            border: "1px solid rgba(238,238,238,0.05)", padding: 24,
          }}>
            <h3 style={{
              fontFamily: "Montserrat", fontWeight: 800, fontSize: "0.8rem",
              letterSpacing: "0.12em", textTransform: "uppercase",
              color: "#eeeeee", marginBottom: 20,
            }}>Resumen</h3>
            ...
            <button onClick={() => navigate("/productos")} style={{
              ...
              padding: "12px", borderRadius: 2, fontFamily: "Montserrat",
      ...
      <style>{`
        .carrito-layout { display: flex; gap: 28px; align-items: flex-start; }
        .carrito-items { flex: 1; }
        .carrito-resumen { width: 260px; flex-shrink: 0; position: sticky; top: 100px; }
        @media (max-width: 768px) {
          .carrito-layout { flex-direction: column; }
          .carrito-resumen { width: 100% !important; position: static !important; }
        }
        @media (max-width: 480px) {
          .carrito-items > div { padding: 12px !important; gap: 10px !important; }
          .carrito-items > div > div:first-child { width: 48px !important; height: 48px !important; }
          .carrito-items > div > div:first-child img { width: 36px !important; height: 36px !important; }
        }
      `}</style>
    </section>
  );
